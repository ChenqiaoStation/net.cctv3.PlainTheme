...
if (!$this->is('attachment')) : ?>
    <div class="w-full p-5 text-gray-900 dark:text-gray-200 bg-white dark:bg-gray-900" x-data="{allow:<?php if ($this->allow('comment')) {
                                                                                                            echo 'true';
                                                                                                        } else {
                                                                                                            echo 'false';
                                                                                                        } ?>}">
        ...
        ?>
            ...
            <li id="li-<?php $comments->theId(); ?>" class="comment-body<?php echo $commentLevelClass; ?>">
                <div id="<?php $comments->theId(); ?>">
                    <div class="h-20 -mt-20 block invisible"></div>
                    <article id="div-<?php $comments->theId(); ?>" class="flex comment-body my-2">
                        <div class="flex-none mr-1" x-data="{k:[],imgurl:'<?php echo letter_avatarx($comments->author); ?>',tx:'<?php echo letter_avatarx($comments->author); ?>'}" x-init="k.type='<?php $k = commenttx($comments->mail, $comments->coid);
                                                                                                                                                                                                    echo $k['type']; ?>';
  ...
  if(k.type=='qq'){
    ...
    imgurl=data.url;});
                            ...
                            <div class="relative">
                                <img no-view class="relative z-10 w-12 object-cover border-2 border-gray-200 rounded-full scrollLoading mr-1" :src="imgurl" alt="<?php echo $comments->author; ?>" x-cloak>
                                <img no-view class="absolute top-0 left-0 w-12 object-cover border-2 border-gray-200 rounded-full scrollLoading mr-1" :src="tx" alt="<?php echo $comments->author; ?>" x-cloak>
                            </div>
        ...
        <div id="comments" class="post-comment">
                        ...
                        <div class="flex text-sm">
                            ...
                            <div class="flex-initial hidden mr-3 mr-avatar md:block">
                                <?php if ($this->user->hasLogin()) : ?>
                                    <img src="<?php tx($this->user->mail); ?>" class="w-12 rounded-full object-cover border-2 border-gray-200 " alt="头像" no-view>
                                <?php else : ?>
                                    <img src="<?php if ($this->remember('mail', true)) {
                                                    tx($this->remember('mail', true));
                                                } else {
                                                    $this->options->themeUrl('img/tx.png');
                                                } ?>" class="w-12 rounded-full object-cover border-2 border-gray-200 " id="pltx" alt="头像" no-view>
                                <?php endif; ?>
                            </div>
                            ...
                            <div class="flex-initial w-full">
                                ...
                                <?php else : ?>
                                    <div class="comment-form-info mt-3">
                                        <div class="grid grid-cols-1 md:grid-cols-<?php if (!empty($this->options->tools) && in_array('spam', $this->options->tools)) {
                                                                                        echo '2';
                                                                                    } else {
                                                                                        echo '3';
                                                                                    } ?> gap-4">
            ...
            <?php else : ?>
                <!--评论已关闭-->
                <div class="w-full flex justify-center text-gray-500 text-sm pt-2 pb-6">
                    评论已关闭
                </div>
            <?php endif; ?>
            ...
            <?php else : ?>
                <div id="nocomment" class="w-full text-center text-gray-500 text-xs pt-2 pb-6">
                    <div class="svg-nocomment w-24 h-24 mx-auto"></div>
                    暂无评论，快来抢沙发
                </div>
            <?php endif; ?>
        </div>
