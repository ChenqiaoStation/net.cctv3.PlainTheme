const linkCard = async (dom, op = "0", divite = "|") => {
  const linkCardTemplate = (url, textContent, image = null) => {
    return `<a target="_blank" data-ajax="false" href="${url}" class="${
      image ? "LinkCard LinkCard-hasImage" : "LinkCard LinkCard-noImage"
    }">
          ${
            image
              ? `<span class="LinkCard-backdrop" style="background-image:url(${image});"></span>`
              : ""
          }
  ...
  const getImage = async (url) => {
    ...
    url =
      "https://blog.zezeshe.com/api/ico?sq=" +
      sitedata.ajax_url +
      "&url=" +
      matchDomain(url)[0] +
      "";
    ...
    if (href) {
      if (href.indexOf(divite) !== -1) {
        ...
        if (
          ...
          ele.parentNode.nodeName == "P" &&
          !ele.classList.contains("shortcode")
        ) {
          pa = 1;
        }
        if (op == "1" || pa == 1) {
          let faviconUrl = await getImage(href);
          if (faviconUrl) {
            template = linkCardTemplate(href, text, faviconUrl);
          } else {
      ...
      if (
        (href.indexOf(divite) !== -1 || op == "1" || pa == 1) &&
        !ele.classList.contains("shortcode")
      ) {
