...
function setshortcode($con, $type = 'post')
{
    ...
    $con = preg_replace('/<h1>(.*?)<\/h1>/i', '<h1 class="text-3xl font-bold text-gray-800 border-b-2 border-gray-200 pb-2 dark:text-white dark:border-gray-700">$1</h1>', $con);
    $con = preg_replace('/<h2>(.*?)<\/h2>/i', '<h2 class="relative pt-3 text-2xl font-bold text-gray-800 dark:text-white">$1</h2>', $con);
    $con = preg_replace('/<h3>(.*?)<\/h3>/i', '<h3 class="relative pt-3 text-xl font-bold text-gray-800 dark:text-white">$1</h3>', $con);
    $con = preg_replace('/<h4>(.*?)<\/h4>/i', '<h4 class="text-lg font-bold text-gray-800 border-b-2 border-gray-100 pb-2 flex flex-row items-center dark:text-white dark:border-gray-700">$1</h4>', $con);
    $con = preg_replace('/<h5>(.*?)<\/h5>/i', '<h5 class="text-base font-bold text-gray-800 border-b-2 border-gray-100 pb-2 flex flex-row items-center dark:text-white dark:border-gray-700">$1</h5>', $con);
    $con = preg_replace('/<h6>(.*?)<\/h6>/i', '<h6 class="text-sm font-bold text-gray-800 border-b-2 border-gray-100 pb-2 flex flex-row items-center dark:text-white dark:border-gray-700">$1</h6>', $con);
...
', $con);
    ...
    $con = preg_replace_callback('#(<p>)?\{bilibili[" *"](av|bv)="(.*?)"[" *"]?\}(<\/p>)?#', 'bilibili', $con);
    //折叠
...
function bilibili($m)
{
    ...
    return '<div  x-data="{bt:\'标题加载中...\',ms:\'描述加载中...\',pic:\'' . theurl . 'img/load.gif\',tname:\'\',view:\'\',author:\'\'}" x-init="fetch(\'' . api('biliinfo') . $m[2] . '=' . $m[3] . '\').then(response => response.json()).then(data=>{
bt=data.data.title; tname=data.data.tname; view=data.data.stat.view;author=data.data.owner.name;if(data.data.dynamic){ms=data.data.dynamic;}else{ms=data.data.desc}pic=\'https://i0.wp.com/\'+data.data.pic;})"><a href="' . $url . '" target="_blank" class="shadow post-item flex rounded mb-5 overflow-hidden p-3.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-left">
<div class="media media-3x2 w-1/3 md:w-1/4 lg:w-1/3 rounded overflow-hidden" style="max-width: 12.5rem;"><img :src="pic" class="media-content h-full w-full object-cover" referrerpolicy="no-referrer" no-view>
    </div>
    <div class="flex flex-col w-full text-gray-700 pl-2 sm:pl-3.5 md:pl-4 py-0.5 dark:text-white">
    <div class="flex-1">
    <div class="text-lg xl:text-xl font-semibold line-2" x-text="bt"></div>
     <div class="hidden md:block"><div class="mt-2 text-sm xl:text-base text-gray-500 dark:text-gray-200 line-2" x-text="ms">描述加载中...</div></div>
   </div>
...
function collapse($m)
{
    ...
    if ($m[3] == 'false') {
        $style = 'max-h-0 py-0 px-4 overflow-y-hidden';
    }
...
function tip($m)
{
    ...
    if (!empty($title)) {
        $title = '<span class="font-semibold text-blue-500 dark:text-blue-400">' . $title . '</span>';
    }
    switch ($type) {
        case "info":
            $color = "bg-blue-500";
...
{
    $api['ico'] = 'https://blog.zezeshe.com/api/ico/?sq=' . rooturl . '&url=';
    $api['biliinfo'] = 'https://blog.zezeshe.com/api/bili/info.php?sq=' . rooturl . '&';
    return $api[$type];
}
