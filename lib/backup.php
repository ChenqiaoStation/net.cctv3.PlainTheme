...
if (isset($_POST["uploadpic"]) && $_POST["uploadpic"] == "上传") {
  ...
  foreach ($file['error'] as $key => $error) {  //遍历处理文件
    if ($error == UPLOAD_ERR_OK) {
      ...
      if (refilex('logo') && $key == 0 && !empty($file['name'])) {
        unlink(refilex('logo'));
      }
      if (refilex('logo-dark') && $key == 1 && !empty($file['name'])) {
        unlink(refilex('logo-dark'));
      }
      if (refilex('tb') && $key == 2 && !empty($file['name'])) {
        unlink(refilex('tb'));
      }
      if (refilex('wx') && $key == 3 && !empty($file['name'])) {
        unlink(refilex('wx'));
      }
      if (refilex('icon') && $key == 4 && !empty($file['name'])) {
        unlink(refilex('icon'));
      }
...
return '<form class="protected setc" action="?' . $name . 'bf" method="post">
